  ...
  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center overflow-hidden pt-8" style={{ background: 'var(--gradient-radial)' }}>
      <img 
        src={logo} 
        alt="Logo" 
        className="absolute top-8 w-16 h-auto md:w-10 lg:w-12" 
        style={{ filter: 'drop-shadow(0 0 3px rgba(255, 255, 255, 0.6))' }}
      />
      <h1 
        className="font-formula text-foreground text-[4rem] leading-tight md:text-7xl lg:text-8xl font-black tracking-normal animate-breathe animate-fade-in px-6 text-center"
        style={{ filter: 'drop-shadow(0 0 5px rgba(255, 255, 255, 0.5))', transform: 'scaleY(1.33)', maxWidth: '90%' }}
      >
      ...
      <a 
        ...
        className="mt-16 px-24 py-2 border border-white rounded-2xl font-formula font-medium text-white text-base md:text-sm cursor-pointer animate-fade-in"
        style={{ 
      ...
      <nav className="absolute bottom-12 flex gap-6 text-sm md:text-xs font-formula font-light">
